...
### —— 把 VPN 隧道 tun0 上的 DNS/QUIC 也交给 Clash —— ###
# 1. DNS 劫持：UDP/TCP 53 → Clash DNS 端口 53
iptables -t nat -I PREROUTING 1 -i tun0 -p udp  --dport 53 -j REDIRECT --to-ports 53
iptables -t nat -I PREROUTING 2 -i tun0 -p tcp  --dport 53 -j REDIRECT --to-ports 53

# 2. QUIC 重定向：UDP/443 → Clash REDIR 端口 7892
iptables -t nat -I PREROUTING 3 -i tun0 -p udp --dport 443 -j REDIRECT --to-ports 7892
